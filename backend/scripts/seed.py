...
def hash_password(password: str) -> str:
    """Хэшируем пароль через bcrypt напрямую (без passlib-прослойки)."""
    salt = bcrypt.gensalt()
...
async def ensure_user(
    session, *, login: str, email: str, password: str, display_name: str, role: UserRole
) -> User:
...
async def ensure_demo_course(session, author: User) -> Course:
    result = await session.execute(select(Course).where(Course.title == "MongoDB для начинающих"))
    ...
    log.info("created demo course (id=%s) with 2 modules, 2 lessons, 1 task", course.course_id)
    return course


async def ensure_achievements(session) -> None:
    # Набор стартовых бейджей.
