...
import { Award, BookOpen, Flame, Target, Trophy, ChevronRight } from "lucide-react";
...
export function DashboardPage() {
  ...
  if (isLoading || !dash) {
    return <div className="max-w-6xl mx-auto px-6 py-10 text-sm text-slate-500">Загрузка…</div>;
  }
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            С возвращением{user?.display_name ? `, ${user.display_name}` : ""}. Продолжайте учиться каждый день.
          </p>
        </div>
        <div className="text-xs text-slate-500">
          Сегодня: <b className="text-slate-900">{new Date().toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" })}</b>
        </div>
      </header>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <section className="bg-white rounded-lg border border-slate-200 p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[15px] font-semibold">Активность за 30 дней</h2>
              <span className="text-xs text-slate-500">{dateRange(dash.activity)}</span>
            </div>
          ...
          <section className="bg-white rounded-lg border border-slate-200 p-5">
            ...
            {dash.current_courses.length === 0 && (
              <div className="text-sm text-slate-500">
                Ещё не приступили ни к одному курсу. <Link to="/" className="text-blue-600 hover:underline">Открыть каталог →</Link>
              </div>
            )}
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="ml-auto text-xs text-slate-500">
                      {c.total_score} <span className="text-slate-400">баллов</span>
                    </span>
                  </div>
                  <div className="text-[14px] font-medium text-slate-900">{c.course_title}</div>
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-[11px] text-slate-500">{c.percent.toFixed(0)}%</span>
        ...
        <div className="space-y-5">
          ...
          <section className="bg-white rounded-lg border border-slate-200 p-5">
            ...
            <div className="space-y-2">
              {dash.achievements.map((a) => (
                ...
                >
                  <div className="text-xl w-7 text-center">{a.icon ?? "🏅"}</div>
                  ...
                  <div className="text-[11px] font-medium text-slate-500">+{a.points}</div>
                </div>
              ))}
            </div>
          </section>
          ...
          {subs && subs.length > 0 && (
            <section className="bg-white rounded-lg border border-slate-200 p-5">
              <h2 className="text-[15px] font-semibold mb-3">Последние попытки</h2>
                  ...
                  <li key={s.submission_id}>
                    ...
                    >
                      <span className={
                        ...
                        (s.status === "correct" ? "bg-emerald-500" :
                         s.status === "timeout" ? "bg-amber-500"   : "bg-rose-500")
                      } />
                      <div className="flex-1 min-w-0">
...
function KpiCard({
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-4">
      ...
      <div className="text-[28px] font-semibold tracking-tight mt-2 leading-none">{value}</div>
...
function ActivityChart({ activity }: { activity: DashboardResponse["activity"] }) {
  ...
  return (
    <div className="h-36 flex items-end gap-[3px]">
      {activity.map((d) => {
        ...
        const h     = total === 0 ? 2 : (total / max) * 100;
        const corrH = total === 0 ? 0 : (d.correct / total) * h;
        const wrH   = total === 0 ? 0 : (d.wrong   / total) * h;

        return (
          <div
            key={d.day}
            className="flex-1 flex flex-col-reverse hover:brightness-110 transition"
            title={`${d.day}: ${d.correct} ✓ / ${d.wrong} ✗`}
          >
            {total > 0 ? (
              <>
                <div style={{ height: `${corrH}%` }} className="bg-emerald-500 rounded-t-[2px]" />
                <div style={{ height: `${wrH}%`   }} className="bg-rose-400" />
              </>
            ) : (
              <div className="h-[2px] bg-slate-200 rounded-t" />
            )}
          </div>
        );
      })}
